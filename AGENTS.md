...
- Полная блокировка (`BarSpec.isBlocking`: percent ≥ 100 или severity строже
  warning) → вместо баров рисуется отсчёт `H:MM` (красный в цветном режиме).
  Отсчёт обновляет **отдельный поминутный `minuteTimer`** в `AppDelegate` из
  кэша `lastBars`, независимо от интервала опроса.
